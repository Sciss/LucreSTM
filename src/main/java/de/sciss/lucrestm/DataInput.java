...
import de.sciss.lucrestm.util.FastInputStream;
 ...
 * @author Mark Hayes
 */
public class DataInput extends FastInputStream {
     ...
     */
    public DataInput(byte[] buffer, int offset, int length) {
     ...
     */
    public DataInput(DataOutput output) {
    ...
    public final String readString()
        ...
        int myOff = off;
        if (available() >= 2 &&
            myBuf[myOff] == DataOutput.NULL_STRING_UTF_VALUE &&
            myBuf[myOff + 1] == 0) {
            skip(2);
            return null;
        } else {
     ...
     * Reads values that were written using {@link DataOutput#writeChar}.
     *
     ...
     * Reads values that were written using {@link DataOutput#writeBoolean}.
     *
     * @return the value read from the buffer.
     *
     * @throws IndexOutOfBoundsException if not enough bytes are available in
     ...
     * Reads values that were written using {@link DataOutput#writeByte}.
     *
     * @return the value read from the buffer.
     *
     * @throws IndexOutOfBoundsException if not enough bytes are available in
     ...
     * Reads values that were written using {@link DataOutput#writeShort}.
     *
     * @return the value read from the buffer.
     *
     * @throws IndexOutOfBoundsException if not enough bytes are available in
     ...
     * Reads values that were written using {@link DataOutput#writeInt}.
     *
     * @return the value read from the buffer.
     *
     * @throws IndexOutOfBoundsException if not enough bytes are available in
     ...
     * Reads values that were written using {@link DataOutput#writeLong}.
     *
     * @return the value read from the buffer.
     *
     * @throws IndexOutOfBoundsException if not enough bytes are available in
     ...
     * Reads values that were written using {@link DataOutput#writeFloat}.
     ...
     * Reads values that were written using {@link DataOutput#writeDouble}.
     ...
     * Reads values that were written using {@link DataOutput#writeBytes}.
     ...
     * Reads values that were written using {@link DataOutput#writeChars}.
     *
     ...
     * written using {@link DataOutput#writeString(String)}.
     *
     * @throws IndexOutOfBoundsException if no null terminating byte is found
     * in the buffer.
     *
    ...
    public final int getStringByteLength()
        ...
        if (available() >= 2 &&
            buf[off] == DataOutput.NULL_STRING_UTF_VALUE &&
            buf[off + 1] == 0) {
            return 2;
        } else {
